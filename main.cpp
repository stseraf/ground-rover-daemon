                                ...
                                const mavlink_command_long_t *cmd)
{
    if (cmd->command == MAV_CMD_COMPONENT_ARM_DISARM) {
        ...
        if (static_cast<uint32_t>(cmd->param1) == MAVLINK_MSG_ID_SYS_STATUS) {
            ...
            send_command_ack(sock, cmd->command, MAV_RESULT_ACCEPTED, cmd->target_system, cmd->target_component);
            send_available_modes(sock, static_cast<uint32_t>(cmd->param2));
        }
        ...
        current_base_mode   = static_cast<uint8_t>(cmd->param1);
        current_custom_mode = static_cast<uint32_t>(cmd->param2);
        send_command_ack(sock, cmd->command, MAV_RESULT_ACCEPTED, cmd->target_system, cmd->target_component);
    }
